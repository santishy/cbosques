...
class UserController extends Controller
{
    ...
    public function quotations(){
      if(Auth::user()->hasRoles(['admin','autorizador']))
        return new QuotationsCollection(Quotation::with('files')->where('cycle_id',session('cycle')->id)->orderBy('id','desc')->paginate(25));
      return new QuotationsCollection(Auth::user()->quotations()->with('files')->where('cycle_id',session('cycle')->id)->orderBy('id','desc')->paginate(25));
    }
